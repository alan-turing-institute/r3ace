...
# Use the systemd-boot EFI boot loader.
  ...
  systemd.services.getting = {
    ...
    script = "getting -p /dev/ttyUSB0 -u 172.0.1.3:8081 -l /home/blue/got_logs -i 0.05 -h /etc/nixos/hosts.yaml https://serving.vm";
    path = with pkgs; [
      inputs.getting.packages."${pkgs.system}".default
    ];
    ...
    serviceConfig = {
      LimitNOFILE = "20000";
      # Allows writing of a log file to /home/ed .
      User = "blue";
  ...
  # Copy required files into the /etc folder of the VM
  environment.etc = {
    "nixos/hosts.yaml" = {
      source = ./hosts.yaml;
      mode = "644";
    ...
    "nixos/configuration.nix" = {
      source = ./todevice/configuration.nix;
      mode = "644";
    };
    "nixos/flake.nix" = {
      source = ./todevice/flake.nix;
      mode = "644";
    };
  };
