...

{
  ...
  systemd.services.blue = {
    ...
    script = "blue -l /home/blue/blue_logs -t 10.0 -e 300 -f 0.8 -i 0.05 -g 169.254.80.236 -r 169.254.93.0 -a /dev/ttyUSB0 -s 5.0";
    path = with pkgs; [
      nftables
      inputs.blue.packages."${pkgs.system}".blue
    ];
  ...
  services.nginx = {
    ...
    virtualHosts."serving.local" = {
      forceSSL = true;
      sslCertificate = "/etc/nixos/nginx_certs/serving.crt";
      sslCertificateKey = "/etc/nixos/nginx_certs/serving.key";
      locations."/" = {
